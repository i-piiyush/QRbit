...
const ShareCard = () => {
  ...
  const viewCardUrl = userId
    ? `${window.location.origin}/user-cards/view-card/${userId}`
    : null;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const currentViews = viewsDocSnap.exists()
          ? viewsDocSnap.data().totalViews || 0
          : 0;
  ...
  const showToast = (message, type = "success") => {
    toast[type](message, {
      ...
      theme: "colored",
      style: {
        backgroundColor: type === "error" ? "#ef4444" : "#16A34A",
        color: "#fff",
      },
    });
  };
          ...
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <p className="text-gray-400">
                Views:{" "}
                <span className="text-green-400 font-medium">{viewCount}</span>
              </p>
            </div>
